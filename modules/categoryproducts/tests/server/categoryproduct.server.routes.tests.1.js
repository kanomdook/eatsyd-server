'use strict';

var should = require('should'),
  request = require('supertest'),
  path = require('path'),
  mongoose = require('mongoose'),
  User = mongoose.model('User'),
  Categoryproduct = mongoose.model('Categoryproduct'),
  Shop = mongoose.model('Shop'),
  Categoryshop = mongoose.model('Categoryshop'),
  express = require(path.resolve('./config/lib/express'));

/**
 * Globals
 */
var app,
  agent,
  credentials,
  user,
  shop,
  categoryshop,
  token,
  categoryproduct;

/**
 * Categoryproduct routes tests
 */
describe('Categoryproduct CRUD tests with token', function () {

  before(function (done) {
    // Get application
    app = express.init(mongoose);
    agent = request.agent(app);

    done();
  });

  beforeEach(function (done) {
    // Create user credentials
    credentials = {
      username: 'username',
      password: 'M3@n.jsI$Aw3$0m3'
    };
    categoryshop = new Categoryshop({
      name: 'อาหารและเคื่องดื่ม'
    });

    shop = new Shop({
      name: 'Shop name',
      name_eng: 'Shop name english',
      detail: 'Shop Detail',
      tel: '0894447208',
      email: 'test@gmail.com',
      facebook: 'facebook.com',
      line: '@lineid',
      address: {
        address: '77/7',
        addressdetail: 'in font of 7-eleven',
        subdistinct: 'Lumlukka',
        distinct: 'Lumlukka',
        province: 'BKK',
        postcode: '12150',
        lat: '13.9338949',
        lng: '100.6827773'
      },
      times: [{
        description: 'all days',
        timestart: '08.00',
        timeend: '20.00',
        days: ['mon', 'thu', 'sun']
      }],
      coverimage: 'https://img.wongnai.com/p/l/2016/11/29/15ff08373d31409fb2f80ebf4623589a.jpg',
      promoteimage: ['http://ed.files-media.com/ud/images/1/22/63943/IMG_7799_Cover.jpg'],
      isactiveshop: false,
      importform: 'manual',
      categories: categoryshop,
      user: user
    });

    token = '';
    // Create a new user
    user = new User({
      firstName: 'Full',
      lastName: 'Name',
      displayName: 'Full Name',
      email: 'test@test.com',
      username: credentials.username,
      password: credentials.password,
      provider: 'local'
    });

    // Save a user to the test db and create new Categoryproduct
    user.save(function () {
      shop.save(function () {
        categoryproduct = {
          name: 'Categoryproduct name',
          priority: 1,
          image: 'data:image/jpeg;base64,',
          user: user,
          shop: shop
        };

      });
      agent.post('/api/auth/signin')
        .send(credentials)
        .expect(200)
        .end(function (signinErr, signinRes) {
          // Handle signin error
          if (signinErr) {
            return done(signinErr);
          }
          signinRes.body.loginToken.should.not.be.empty();
          token = signinRes.body.loginToken;
          done();
        });

    });
  });

  it('should be have Token logged in', function (done) {
    token.should.not.be.empty();
    done();
  });

  it('should be able to save a Categoryproduct if logged in with token', function (done) {
    // Save a new Categoryproduct
    agent.post('/api/categoryproducts')
      .set('authorization', 'Bearer ' + token)
      .send(categoryproduct)
      .expect(200)
      .end(function (categoryproductSaveErr, categoryproductSaveRes) {
        // Handle categoryproduct save error
        if (categoryproductSaveErr) {
          return done(categoryproductSaveErr);
        }

        // Get a list of categoryproduct
        agent.get('/api/categoryproducts')
          .end(function (categoryproductsGetErr, categoryproductsGetRes) {
            // Handle categoryproduct save error
            if (categoryproductsGetErr) {
              return done(categoryproductsGetErr);
            }

            // Get categoryproducts list
            var categoryproducts = categoryproductsGetRes.body;

            // Set assertions
            (categoryproducts[0].name).should.match('Categoryproduct name');

            // Call the assertion callback
            done();
          });
      });
  });

  it('should be able to update a Categoryproduct if logged in with token', function (done) {
    // Save a new categoryproducts
    agent.post('/api/categoryproducts')
      .set('authorization', 'Bearer ' + token)
      .send(categoryproduct)
      .expect(200)
      .end(function (categoryproductSaveErr, categoryproductSaveRes) {
        // Handle categoryproduct save error
        if (categoryproductSaveErr) {
          return done(categoryproductSaveErr);
        }

        categoryproduct.name = "test categoryproduct";
        agent.put('/api/categoryproducts/' + categoryproductSaveRes.body._id)
          .set('authorization', 'Bearer ' + token)
          .send(categoryproduct)
          .expect(200)
          .end(function (categoryproductUpdateErr, categoryproductUpdateRes) {
            // Handle categoryproduct save error
            if (categoryproductUpdateErr) {
              return done(categoryproductUpdateErr);
            }

            // Get a list of categoryproducts
            agent.get('/api/categoryproducts')
              .end(function (categoryproductsGetErr, categoryproductsGetRes) {
                // Handle categoryproducts save error
                if (categoryproductsGetErr) {
                  return done(categoryproductsGetErr);
                }

                // Get categoryproducts list
                var categoryproducts = categoryproductsGetRes.body;

                // Set assertions
                (categoryproducts[0].name).should.equal('test categoryproduct');

                // Call the assertion callback
                done();
              });
          });
      });
  });

  it('should be able to delete a Categoryproduct if logged in with token', function (done) {
    // Save a new categoryproducts
    agent.post('/api/categoryproducts')
      .set('authorization', 'Bearer ' + token)
      .send(categoryproduct)
      .expect(200)
      .end(function (categoryproductSaveErr, categoryproductSaveRes) {
        // Handle categoryproduct save error
        if (categoryproductSaveErr) {
          return done(categoryproductSaveErr);
        }

        agent.delete('/api/categoryproducts/' + categoryproductSaveRes.body._id)
          .set('authorization', 'Bearer ' + token)
          .send(categoryproduct)
          .expect(200)
          .end(function (categoryproductUpdateErr, categoryproductUpdateRes) {
            // Handle categoryproduct save error
            if (categoryproductUpdateErr) {
              return done(categoryproductUpdateErr);
            }
            // Get a list of categoryproduct
            agent.get('/api/categoryproducts')
              .end(function (categoryproductsGetErr, categoryproductsGetRes) {
                // Handle categoryproduct save error
                if (categoryproductsGetErr) {
                  return done(categoryproductsGetErr);
                }

                // Get categoryproduct list
                var categoryproducts = categoryproductsGetRes.body;

                // Set assertions
                (categoryproducts.length).should.match(0);

                // Call the assertion callback
                done();
              });
          });
      });
  });

  it('should be able to get List a Categoryproduct if logged in with token', function (done) {
    // Save a new categoryproducts
    agent.post('/api/categoryproducts')
      .set('authorization', 'Bearer ' + token)
      .send(categoryproduct)
      .expect(200)
      .end(function (categoryproductSaveErr, categoryproductSaveRes) {
        // Handle categoryproduct save error
        if (categoryproductSaveErr) {
          return done(categoryproductSaveErr);
        }

        // Get a list of categoryproducts
        agent.get('/api/categoryproducts')
          .end(function (categoryproductsGetErr, categoryproductsGetRes) {
            // Handle categoryproducts save error
            if (categoryproductsGetErr) {
              return done(categoryproductsGetErr);
            }

            // Get Products list
            var categoryproducts = categoryproductsGetRes.body;

            // Set assertions
            (categoryproducts.length).should.match(1);
            (categoryproducts[0].name).should.match(categoryproduct.name);
            (categoryproducts[0].image).should.match(categoryproduct.image);
            (categoryproducts[0].priority).should.match(categoryproduct.priority);
            done();

          });
      });
  });

  it('should be able to get By ID a Categoryproduct if logged in with token', function (done) {
    // Save a new Categoryproduct
    agent.post('/api/categoryproducts')
      .set('authorization', 'Bearer ' + token)
      .send(categoryproduct)
      .expect(200)
      .end(function (categoryproductSaveErr, categoryproductSaveRes) {
        // Handle Categoryproduct save error
        if (categoryproductSaveErr) {
          return done(categoryproductSaveErr);
        }

        var categoryproductObj = categoryproductSaveRes.body;
        agent.get('/api/categoryproducts/' + categoryproductSaveRes.body._id)
          .set('authorization', 'Bearer ' + token)
          .end(function (categoryproductGetErr, categoryproductsGetRes) {
            // Handle Categoryproduct save error
            if (categoryproductGetErr) {
              return done(categoryproductGetErr);
            }
            // Get Categoryproduct list
            var categoryproducts = categoryproductsGetRes.body;

            // Set assertions
            // (products[0].user.loginToken).should.equal(token);
            (categoryproducts.name).should.match('Categoryproduct name');
            (categoryproducts.shop.name).should.match(shop.name);

            done();
          });
      });
  });

  it('should be able to get List a Categoryproduct by Shop if logged in with token', function (done) {
    var CateObj = new Categoryproduct(categoryproduct);
    // Get a list of Categoryproduct
    CateObj.save();
    agent.get('/api/categoryproductsbyshop/' + shop.id)
      .set('authorization', 'Bearer ' + token)
      .end(function (categoryproductsGetErr, categoryproductsGetRes) {
        // Handle Categoryproduct save error
        if (categoryproductsGetErr) {
          return done(categoryproductsGetErr);
        }

        // Get Categoryproduct list
        var categoryproducts = categoryproductsGetRes.body;

        // Set assertions
        (categoryproducts.items.length).should.match(1);
        (categoryproducts.items[0].name).should.match(categoryproduct.name);
        (categoryproducts.items[0].image).should.match(categoryproduct.image);
        (categoryproducts.items[0].priority).should.match(categoryproduct.priority);
        
        done();
        
      });
  });


  afterEach(function (done) {
    User.remove().exec(function () {
      Shop.remove().exec(function () {
        Categoryproduct.remove().exec(done);
      });
    });
  });
});
